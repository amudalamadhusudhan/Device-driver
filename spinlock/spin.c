...
static int char_open(struct inode *inode, struct file *file)
{
    pr_info("spin driver open function called\n");
    return 0;
}
static int char_release(struct inode *inode, struct file *file)
{
    pr_info("spin driver realease function called\n");
    return 0;
}
static ssize_t char_read(struct file *filp, char __user *buf, size_t len, loff_t *off)
{
    printk("spin driver read function called\n");
    int r;
...
{
    spin_lock(&spinlock);
    printk("\nspin driver write function called\n");
    int kbuff[2];
    int r, n1, n2;
...
static int __init driverI(void)
{
    alloc_chrdev_region(&dev, 0, 3, "spin char dynamic driver");
    if ((alloc_chrdev_region(&dev, 0, 3, "spin char  dynamic driver")) < 0)
    {
        printk(KERN_ALERT "spin driver inserted unsussfuly\n");
        return -1;
    }
    printk(KERN_INFO "\n major =%d minor=%d\n", MAJOR(dev), MINOR(dev));
    ...
    printk(KERN_ALERT "spin char device driver inserted successfuly\n");
...
static void __exit driverE(void)
{
    ...
    printk(KERN_ALERT "\n spin char  dynamic device driver removed successfully\n");
}
